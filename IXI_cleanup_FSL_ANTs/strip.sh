...
if [ -n "$2" ]; then # Should get input dir and output dir as params
...
(ls $src_path/*.nii.gz | xargs -d '\n' -n 1 basename) | parallel --jobs 6 fslreorient2std $src_path/{} tmp/fslreorient2std/{} >> tmp/log.txt
